...
*/
	...
	*tunings {
		var tuningfilepath = Platform.userAppSupportDir+/+"scl_user/_tuningmenu.scd";
		var tuningsarray, tuningfile, line;
		
		tuningsarray = [];
		line = 0;
		
		if(File.exists(tuningfilepath), {
			tuningfile = File.open( tuningfilepath, "r" );
			while ({ line.isNil.not }, {
				line = tuningfile.getLine; 
				if(line.isNil.not, { tuningsarray = tuningsarray.add(line) });
			});
			tuningfile.close;
		}, {
			tuningsarray = ["et12", "pythagorean", "just", "sept1", "sept2", "mean4", "mean5", "mean6", "kirnberger", "werckmeister", "vallotti", "young", "reinhard", "wcHarm", "wcSJ", "et19", "et31"]; // just some examples
		});
		[\tuningsarray, tuningsarray].postln;
		^tuningsarray;
